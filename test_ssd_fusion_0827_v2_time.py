...
from ssd_fusion_0827_v2_E_compute_time import build_ssd
...
def test_net(save_folder, net, detector, cuda, testset, transform, max_per_image=300, thresh=0.005,obj_thresh=0.01):
    ...
    for i in range(num_images):
        ...
        _t['im_detect'].tic()
        out,ratio,se_ratio = net(x)      # forward pass
        ...
        for i in range(0,len(se_ratio)):
            se_ratios[i] += se_ratio[i]
    ...
    name = ['backbone','fusion1','extra','fusion2','fusion3','fusion4','fusion5','prediction']
    for i in range(0,len(ratios)):
        print( name[i] + ': %.4f'%(ratios[i]*1.0/4952),end='  ' )
    name1 = ['se0','se1','se2','se3','se4']
    for i in range(0,len(se_ratios)):
        print( name1[i] + ':%.4f'%(se_ratios[i]*1.0/4952),end = ' ')
