...
parser.add_argument('--log_file',help='log of trainging file')
parser.add_argument('--device', type=int, help='gpus')
...
criterion = RefineMultiBoxLoss(num_classes, 0.5, True, 0, True, 3, 0.5, False,0.9)
...
def train():
    ...
    print('VOC length:'+str(len(dataset)))
    print('epoch_size:'+str(epoch_size))
    print('max _ iter '+ str(max_iter))
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % epoch_size == 0:
            ...
            if (epoch % 10 == 0 and epoch > 0) or (epoch % 5 ==0 and epoch > 200):
                torch.save(net.state_dict(), 'weights/'+model_name+'/ssd300_COCO_'+repr(iteration)+'_epoches_'+
                           repr(epoch) + '.pth')
                '''
                    '.pth'
        ...
        if (iteration !=0 and iteration % 2000 ==0):
            torch.save(net.state_dict(),'weights/'+model_name+'/ssd300_COCO_'+repr(iteration)+'.pth')
        ...
        if iteration % 10 == 0:
            print('Epoch:' + repr(epoch) + ' || epochiter: ' + repr(iteration % epoch_size) + '/' + repr(epoch_size)
                ...
                loss_l,loss_c,loss_obj) + '||Total loss: %.4f||'%(loss_l+loss_c) +
