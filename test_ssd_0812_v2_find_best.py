...
from ssd_fusion_0812_v2 import build_ssd
...
if __name__ == '__main__':
    ...
    for i in range(start_iter,end_iter+step,step):
        trained_model = 'weights/fusion_0812_v2_32batch/ssd300_COCO_'+str(i)+'.pth'
        ...
        if args.cuda:
            ...
            net = net.cpu()
        net.eval()
        mAP,detect_time,nms_time,fps1,fps2, = test_net(save_folder, net, detector, args.cuda, testset,
             BaseTransform(net.size, rgb_means, (2, 0, 1)),
             top_k, thresh=0.01)
        if mAP > best_ap : 
            best_ap = mAP 
            best_iter = i
            print('iter: {} mAP: {} det_time:{} , nms_time:{} fps1: {} fps2:{} best_ap:{} bets_inter:{}'.format(i,mAP,detect_time, nms_time,fps1,fps2,best_ap,best_iter))

        else:
            print('iter: {} mAP: {} det_time:{} , nms_time:{} fps1: {} fps2:{} best_ap:{} bets_inter:{}'.format(i,mAP,detect_time, nms_time,fps1,fps2,best_ap,best_iter))
        output_file.write('iter: {} mAP: {} det_time:{} , nms_time:{} fps1: {} fps2:{} best_ap:{} bets_inter:{}'.format(i,mAP,detect_time, nms_time,fps1,fps2,best_ap,best_iter))
        output_file.flush()
