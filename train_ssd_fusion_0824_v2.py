...
from data import VOCroot, COCOroot, VOC_300, VOC_512, AnnotationTransform, VOCDetection, detection_collate, BaseTransform, preproc
from layers.modules import RefineMultiBoxLoss
from layers.functions import PriorBox
import time
from ssd_fusion_0824_v2_E import build_ssd
...
net = build_ssd('train', img_dim, num_classes)
#print(net)
if args.resume_net == None:
    base_weights = torch.load(args.basenet,map_location = lambda storage,loc:storage  )
...
criterion = RefineMultiBoxLoss(num_classes, 0.5, True, 0, True, 3, 0.5, False,0.01)
priorbox = PriorBox(cfg)
priors = priorbox.forward()
priors = Variable(priors,volatile=True)
priors = priors.cuda()
'''
with torch.no_grad():
    priors = priorbox.forward()
...
def train():
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if (iteration != 0 ) and (iteration % 2000) == 0 :
            torch.save(net.state_dict(),args.save_folder+model_name+'/'+'ssd300_coco_'+str(iteration)+'.pth')
        ...
        if iteration % 10 == 0:
            print('Epoch:' + repr(epoch) + ' || epochiter: ' + repr(iteration % epoch_size) + '/' + repr(epoch_size)
                ...
                loss_l,loss_c,loss_obj) + 'Total Loss : %.4f||'%(loss_l+loss_c+loss_obj)+
