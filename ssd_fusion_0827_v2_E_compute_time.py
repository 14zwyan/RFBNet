...
from layers import *
from data import VOCroot, COCOroot, VOC_300, VOC_512,  AnnotationTransform, detection_collate, BaseTransform, preproc
import os
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        self.up_fc7 = nn.ConvTranspose2d( 1024,256, kernel_size=2,stride=2,padding=0)
        self.up_fc7_conv = nn.Conv2d(256,256,stride=1,kernel_size=3,padding=1)
        #self.down_conv3_1 = nn.Conv2d(256,512,stride=1,kernel_size=1)
        self.down_conv3_1 = nn.Conv2d(256,256,stride=1,kernel_size=1)
        self.down_conv3_1_conv = nn.Conv2d(256,256,stride=1,kernel_size=3,padding=1)
        self.batch_norm0 = nn.BatchNorm2d(256)
        self.batch_norm1 = nn.BatchNorm2d(256)
        self.batch_norm2 = nn.BatchNorm2d(256)
        self.fus_conv0 = nn.Conv2d(512,256,stride=1,kernel_size=1)
        self.fus_conv0_1 = nn.Conv2d(256,256,stride=1,kernel_size=3,padding=1)
        self.se0 = Local_Global_Attention_Hybrid(256*3,256,16)
        self.scale0 = ScaleLayer(init_value=1)

        self.down_conv4_1 = nn.Conv2d(512,256,stride=1,kernel_size=1)
        self.down_conv4_1_conv = nn.Conv2d(256,256,stride=1,kernel_size=3,padding=1)
        self.up_conv6_2 = nn.ConvTranspose2d(512,256,kernel_size=3,stride=2,padding=1)
        self.up_conv6_2_conv = nn.Conv2d(256,256,kernel_size=3,stride=1,padding=1)
        self.batch_norm3 = nn.BatchNorm2d(256)
        self.batch_norm4 = nn.BatchNorm2d(256)
        self.batch_norm5 = nn.BatchNorm2d(256)
        self.fus_conv1 = nn.Conv2d(1024,256,kernel_size=1,stride=1,padding=0)
        self.fus_conv1_1 = nn.Conv2d(256,256,kernel_size=3,stride=1,padding=1)
        self.se1= Local_Global_Attention_Hybrid(256*3,256,16)
        self.scale1 = ScaleLayer(init_value=1)

        self.down_fc7 = nn.Conv2d(1024,256,stride=1,kernel_size=1,padding=0)
        self.down_fc7_conv = nn.Conv2d(256,256,stride=1,kernel_size=3,padding=1)
        self.up_conv7_2 = nn.ConvTranspose2d(256,256,stride=2,kernel_size=2,padding=0)
        self.up_conv7_2_conv = nn.Conv2d(256,256,kernel_size=3,stride=1,padding=1)
        self.batch_norm6 = nn.BatchNorm2d(256)
        self.batch_norm7 = nn.BatchNorm2d(256)
        self.batch_norm8 = nn.BatchNorm2d(256)
        self.fus_conv2 = nn.Conv2d(512,256,stride=1,kernel_size=1)
        self.fus_conv2_1 = nn.Conv2d(256,256,kernel_size=3,stride=1,padding=1)
        self.se2 = Local_Global_Attention_Hybrid(256*3,256,16)
        ...
        self.se3= Local_Global_Attention_Hybrid(256*3,256,16)
        ...
        self.se4 = Local_Global_Attention_Hybrid(256*3,256,16)
    ...
    def forward(self, x):
        ...
        """
        _t  = {'backbone':Timer(),'fusion1':Timer(),'extra':Timer(),
                'fusion2':Timer(),'fusion3':Timer(),'fusion4':Timer(),'fusion5':Timer(),
                'prediction': Timer()}
        _t_se = {'se0':Timer(),'se1':Timer(),'se2':Timer(),'se3':Timer()}
        ...
        conv4_3_1 = F.relu( self.fus_conv0(conv4_3) )
        conv4_3_1 = F.relu( self.fus_conv0_1(conv4_3_1) )
        conv4_3_bn = self.batch_norm1( conv4_3_1 )
        fc7_up = self.batch_norm2( fc7_up )

        conv4_3_bn = torch.cat([ conv3_1_down,conv4_3_bn,fc7_up ],dim=1)
        _t_se['se0'].tic()
        conv4_3_bn = self.se0(conv4_3_bn)
        se_time.append(_t_se['se0'].toc())
        [conv3_1_down,conv4_3_bn,fc7_up] = torch.split(conv4_3_bn,256,dim=1)

        conv4_3_bn= conv4_3_bn + fc7_up + conv3_1_down
        conv4_3_1 = self.L2Norm( conv4_3_bn )

        times.append(_t['fusion1'].toc())
        ...
        fc7_1 = F.relu(self.fus_conv1(fc7))
        fc7_1 = F.relu(self.fus_conv1_1(fc7_1))
        fc7_bn = self.batch_norm4( fc7_1 )
        conv6_2_up = self.up_conv6_2( conv6_2 )
        conv6_2_up = self.up_conv6_2_conv(conv6_2_up)
        conv6_2_up  = self.batch_norm5(conv6_2_up)

        fc7_bn = torch.cat([ conv4_1_down,fc7_bn,conv6_2_up ],dim=1)
        _t_se['se1'].tic()
        fc7_bn = self.se1(fc7_bn)
        se_time.append(_t_se['se1'].toc())
        [conv4_1_down,fc7_bn,conv6_2_up] = torch.split(fc7_bn,256,dim=1)

        fc7_1 = fc7_bn + conv4_1_down + conv6_2_up
        fc7_1 = self.scale1( fc7_1 )
        times.append(_t['fusion2'].toc())
        ...
        conv6_2_1 = F.relu(self.fus_conv2(conv6_2))
        conv6_2_1 = F.relu(self.fus_conv2_1(conv6_2_1))
        conv6_2_bn = self.batch_norm7( conv6_2_1 )
        conv7_2_up = self.up_conv7_2( conv7_2 )
        conv7_2_up = self.up_conv7_2_conv( conv7_2_up)
        conv7_2_up = self.batch_norm8(conv7_2_up)

        conv6_2_bn = torch.cat([ fc7_down,conv6_2_bn,conv7_2_up ],dim=1)
        _t_se['se2'].tic()
        conv6_2_bn = self.se2(conv6_2_bn)
        se_time.append(_t_se['se2'].toc())
        [fc7_down,conv6_2_bn,conv7_2_up] = torch.split(conv6_2_bn,256,dim=1)

        conv6_2_1 = conv6_2_bn + fc7_down + conv7_2_up
        conv6_2_1 = self.scale2(conv6_2_1)
        times.append(_t['fusion3'].toc())
        ...
        conv7_2_bn = torch.cat([ conv6_2_down,conv7_2_bn,conv8_2_up ],dim=1)
        _t_se['se3'].tic()
        conv7_2_bn = self.se3(conv7_2_bn)
        se_time.append(_t_se['se3'].toc())
        [con6_2_down,conv7_2_bn,conv8_2_up] = torch.split(conv7_2_bn,256,dim=1)

        conv7_2_1 = conv7_2_bn + conv8_2_up + conv6_2_down
        conv7_2_1 = self.scale3( conv7_2_1 )
        times.append(_t['fusion4'].toc())
        ...
        conv8_2_bn = torch.cat([ conv7_2_down,conv8_2_bn,conv9_2_up ],dim=1)
        conv8_2_bn = self.se4(conv8_2_bn)
        [conv7_2_down,conv8_2_bn,conv9_2_up] = torch.split(conv8_2_bn,256,dim=1)

        conv8_2_1 = conv8_2_bn + conv9_2_up + conv7_2_down
        conv8_2_1 = self.scale4( conv8_2_1 )
        times.append(_t['fusion5'].toc())
        ...
        times.append(_t['prediction'].toc())
        ...
        for name in _t:
            time = times[i]
            print(name + ": %4f"%(time/total_time*100),end=' ')
            ratio.append( (time/total_time)*100)
            i += 1
        print()
        i = 0 
        se_ratio = list()
        for name in _t_se:
            time = se_time[i]
            print(name+':%4f'%(time/total_time*100),end='  ')
            se_ratio.append( time/total_time*100 )
            i += 1
        print()
